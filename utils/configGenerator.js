// 설정 생성 관련 유틸리티 함수들
const ConfigGeneratorUtils = {
    // CPU 프리셋 적용 함수
    applyCpuPreset: (preset, setCpuCores, setCpuThreads) => {
      switch (preset) {
        case "intel-i5":
          setCpuCores(6);
          setCpuThreads(12);
    ...
    // GPU 프리셋 적용 함수
    applyGpuPreset: (preset, setGpuTier) => {
      switch (preset) {
        case "nvidia-low":
        case "amd-low":
          setGpuTier("low");
    ...
    // 설정 생성 함수
    generateConfig: (cpuThreads, gpuTier, ram, unityVersion, platform) => {
  ...
  window.ConfigGeneratorUtils = ConfigGeneratorUtils;